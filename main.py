...
def scan_local_dump_files():
    """
    Scan file .json/.md/.docx di input_dir (fallback root).
    Skip folder backups/, public/, inputs/ nested.
    """
    ...
    for ext in extensions:
        ...
        # Kalau input_dir != "." (yaitu inputs/), gak scan root
        # Kalau input_dir == ".", udah ke-cover
    return sorted(found)
...
def _get_menu_context() -> dict:
    ...
    # NEW: Count readers & comics
    for subdir, key in (("readers", "readers_count"), ("comics", "comics_count")):
        try:
            folder = os.path.join(PUBLIC_DIR, subdir)
...
def _print_post_render_hint():
    ...
    if t.enabled:
        ...
        print()
        print(f"  {C.GRAY}Baca panduan lengkap: menu {C.ACCENT}[5] Tutorial{C.RESET}")
        print(f"  {C.ACCENT_DIM}{'─' * 56}{C.RESET}")
    else:
        print("  " + "─" * 56)
        ...
        print()
        print("  Baca panduan lengkap: menu [5] Tutorial")
        print("  " + "─" * 56)
...
def handle_local_render() -> bool:
    ...
    if not files:
        print_warn(f"Gak ada file .json / .md / .docx di folder '{input_dir}'.")
        print()
        print_info("Taruh file backup chat lu di folder itu dulu.")
        ...
        for idx, f in enumerate(files, 1):
            ...
            print_numbered(idx, f"{os.path.basename(f)}  ({size_kb:.1f} KB)")
...
def handle_tutorial():
    ...
    if t.enabled:
        ...
        print(f"  {C.GRAY}💡 Udah jalan, cuma butuh fitur? Skip ke {C.WHITE}PDF / COMIC{C.RESET}{C.GRAY}.{C.RESET}")
    ...
    # --- SECTION 2: PDF / COMIC IMPORT (NEW) ---
    _section("PDF / COMIC IMPORT", icon="📚")
    print()
    _line("GhostWriter bisa import:")
    _line("• PDF ebook (hasil scan / digital)", indent=4)
    _line("• Comic ZIP (manga, webtoon, comic strip)", indent=4)
    print()
    _line("Cara pakai:")
    print()
    if t.enabled:
        print(f"  {C.ACCENT_DIM}1.{C.RESET} {C.WHITE}Taruh file di folder {C.ACCENT}inputs/{C.RESET}")
    else:
        print("  1. Taruh file di folder inputs/")
    print()
    if t.enabled:
        print(f"  {C.ACCENT_DIM}2.{C.RESET} {C.WHITE}Dari menu utama:{C.RESET}")
        print(f"     {C.GRAY}• Pilih {C.ACCENT}[3]{C.RESET} {C.GRAY}buat PDF{C.RESET}")
        print(f"     {C.GRAY}• Pilih {C.ACCENT}[4]{C.RESET} {C.GRAY}buat Comic ZIP{C.RESET}")
    else:
        print("  2. Dari menu utama:")
        print("     • Pilih [3] buat PDF")
        print("     • Pilih [4] buat Comic ZIP")
    print()
    if t.enabled:
        print(f"  {C.ACCENT_DIM}3.{C.RESET} {C.WHITE}Buka di browser:{C.RESET}")
        _cmd("http://localhost:8000/readers/<slug>/")
        _cmd("http://localhost:8000/comics/<slug>/")
    else:
        print("  3. Buka di browser:")
        print("     http://localhost:8000/readers/<slug>/")
        print("     http://localhost:8000/comics/<slug>/")
    print()
    _hint("💡 PDF di-render jadi image (JPEG 150 DPI) — cocok buat scan.")
    _info("Comic ZIP di-extract natural sort — urutan halaman otomatis bener.")

    # --- SECTION 3: CARA BUKA DI BROWSER ---
    ...
    if t.enabled:
        for key, desc in shortcuts_chat:
            print(f"  {C.ACCENT}{key:<10}{C.RESET} {C.WHITE}{desc}{C.RESET}")
    else:
        for key, desc in shortcuts_chat:
            print(f"  {key:<10} {desc}")
    print()
    _line("Reader (ebook / comic):")
    shortcuts_reader = [
        ("?", "Buka panel keyboard shortcuts"),
    ...
    if t.enabled:
        for key, desc in shortcuts_reader:
            print(f"  {C.ACCENT}{key:<10}{C.RESET} {C.WHITE}{desc}{C.RESET}")
    else:
        for key, desc in shortcuts_reader:
            print(f"  {key:<10} {desc}")
    ...
    print()
    if t.enabled:
        print(f"  {C.YELLOW}🟡 PDF / Comic Import{C.RESET}")
        print(f"  {C.ACCENT_DIM}{'─' * 54}{C.RESET}")
    else:
        print("  🟡 PDF / Comic Import")
        print("  " + "─" * 54)

    print()
    _hint("Q: PDF gak ke-render / error poppler?")
    _line("A: Install Poppler:", indent=4)
    _line("   Termux : pkg install poppler", indent=6)
    _line("   Ubuntu : sudo apt install poppler-utils", indent=6)
    print()
    _hint("Q: Import lambat buat PDF gede?")
    _line("A: Normal — 100 halaman bisa 5-15 menit.", indent=4)
    _line("   Tergantung device & DPI.", indent=4)
    print()
    _hint("Q: Gambar di comic gak ke-render?")
    _line("A: Cek console browser (F12) — biasanya path salah.", indent=4)
    _line("   Atau format gak didukung (.avif butuh browser modern).", indent=4)
    ...
    if t.enabled:
        ...
        print(f"  {C.GRAY}• Baca ulang: {C.WHITE}[5]{C.RESET} {C.GRAY}dari menu utama{C.RESET}")
        ...
        print("  • Baca ulang: [5] dari menu utama")
...
def print_main_menu():
    ...
    if t.enabled:
        ...
        print()
        print(f"  {C.ACCENT}[3]{C.RESET} {C.ACCENT}{bullet}{C.RESET}  {C.WHITE}Import Ebook PDF{C.RESET}")
        print(f"       {C.GRAY}Image mode — cocok buat scan{C.RESET}")
        print()
        print(f"  {C.ACCENT}[4]{C.RESET} {C.ACCENT}{bullet}{C.RESET}  {C.WHITE}Import Comic ZIP{C.RESET}")
        print(f"       {C.GRAY}Manga, webtoon, comic strip{C.RESET}")
        print()
        print(f"  {C.ACCENT}[5]{C.RESET} {C.ACCENT}{bullet}{C.RESET}  {C.WHITE}Tutorial & Panduan{C.RESET}")
        ...
        if ctx["convo_count"] > 0:
            ctx_parts.append(
                f"{C.GRAY}📚{C.RESET} {C.WHITE}{ctx['convo_count']}{C.RESET} {C.GRAY}chat{C.RESET}"
            )
        if ctx["readers_count"] > 0:
            ctx_parts.append(
                f"{C.GRAY}📖{C.RESET} {C.WHITE}{ctx['readers_count']}{C.RESET} {C.GRAY}ebook{C.RESET}"
            )
        if ctx["comics_count"] > 0:
            ctx_parts.append(
                f"{C.GRAY}🎨{C.RESET} {C.WHITE}{ctx['comics_count']}{C.RESET} {C.GRAY}comic{C.RESET}"
            )
        ctx_parts.append(
        ...
        if ctx["files_count"] == 0 and ctx["readers_count"] == 0 and ctx["comics_count"] == 0:
            print(f"  {C.YELLOW}💡{C.RESET}  {C.GRAY}Taruh file di {C.WHITE}inputs/{C.RESET} {C.GRAY}buat mulai.{C.RESET}")
        ...
        print()
        print(f"  [3] {bullet}  Import Ebook PDF")
        print(f"       Image mode — cocok buat scan")
        print()
        print(f"  [4] {bullet}  Import Comic ZIP")
        print(f"       Manga, webtoon, comic strip")
        print()
        print(f"  [5] {bullet}  Tutorial & Panduan")
        ...
        ctx_parts = []
        if ctx["files_count"] > 0:
            ctx_parts.append(f"📁 {ctx['files_count']} file")
            ctx_parts.append(f"💾 {_format_size(ctx['files_size'])}")
        if ctx["convo_count"] > 0:
            ctx_parts.append(f"📚 {ctx['convo_count']} chat")
        if ctx["readers_count"] > 0:
            ctx_parts.append(f"📖 {ctx['readers_count']} ebook")
        if ctx["comics_count"] > 0:
            ctx_parts.append(f"🎨 {ctx['comics_count']} comic")
        ...
        if ctx["files_count"] == 0 and ctx["readers_count"] == 0 and ctx["comics_count"] == 0:
            print("  💡 Taruh file di inputs/ buat mulai.")
        print("  " + sep * 55)
...
def run_wizard():
    ...
    while True:
        print_main_menu()
        pilihan = _prompt("Pilih menu [0-5]", default="0")
        ...
        if pilihan == "1":
            ...
            _safe_run(handle_pdf_import, "Import PDF")
            _wait_enter()
        elif pilihan == "4":
            _safe_run(handle_comic_import, "Import Comic")
            _wait_enter()
        elif pilihan == "5":
            _safe_run(handle_tutorial, "Tutorial")
        else:
            print_warn(f"Pilihan '{pilihan}' gak valid. Pilih [0-5].")
